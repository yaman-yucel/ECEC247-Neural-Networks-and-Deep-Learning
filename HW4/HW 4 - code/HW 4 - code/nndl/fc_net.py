...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            if i == 1:
                if self.use_batchnorm:
                    h_tmp, cache_h_tmp = affine_batchnorm_relu_forward(X, self.params["W" + str(i)],self.params["b" + str(i)], self.params['gamma' + str(i)], self.params['beta' + str(i)], self.bn_params[i-1])
                    cache_h.append(cache_h_tmp)
                else:
                    h_tmp, cache_h_tmp = affine_relu_forward(X, self.params["W" + str(i)],self.params["b" + str(i)])
                    cache_h.append(cache_h_tmp)
                
                if self.use_dropout:
                    h_tmp, cache_dropout_tmp = dropout_forward(h_tmp, self.dropout_param)
                    cache_dropout.append(cache_dropout_tmp)
            elif i == self.num_layers:
                scores, cache_h_tmp = affine_forward(h_tmp, self.params["W" + str(i)],self.params["b" + str(i)])
                cache_h.append(cache_h_tmp)
            else:
                if self.use_batchnorm:
                    h_tmp, cache_h_tmp = affine_batchnorm_relu_forward(h_tmp, self.params["W" + str(i)],self.params["b" + str(i)], self.params['gamma' + str(i)], self.params['beta' + str(i)], self.bn_params[i-1])
                    cache_h.append(cache_h_tmp)
                else:
                    h_tmp, cache_h_tmp = affine_relu_forward(h_tmp, self.params["W" + str(i)],self.params["b" + str(i)]) 
                    cache_h.append(cache_h_tmp)
                if self.use_dropout:
                    h_tmp, cache_dropout_tmp = dropout_forward(h_tmp, self.dropout_param)
                    cache_dropout.append(cache_dropout_tmp)
        ...
        for i in range(self.num_layers,0,-1):
            ...
            if i == self.num_layers:
                d_h_tmp, grads["W" + str(i)], grads["b" + str(i)] = affine_backward(d_scores, cache_h[i - 1])
            else:
                if self.use_dropout:
                    d_h_tmp = dropout_backward(d_h_tmp, cache_dropout[i-1])
                if self.use_batchnorm:
                    d_h_tmp, grads["W" + str(i)], grads["b" + str(i)], grads['gamma' + str(i)],grads['beta' + str(i)] = affine_batchnorm_relu_backward(d_h_tmp, cache_h[i - 1])
                else: 
                    d_h_tmp, grads["W" + str(i)], grads["b" + str(i)] = affine_relu_backward(d_h_tmp, cache_h[i - 1])
